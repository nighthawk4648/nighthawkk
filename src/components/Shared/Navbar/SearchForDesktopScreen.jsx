          ...
          {isSearchOpen && (
            <div className="origin-top-right absolute right-0 mt-2 w-full  max-h-[550px] overflow-y-auto  shadow-lg bg-white z-[9000]  h-fit ">
              <div className="py-1 ">
                ...
                searchData?.data?.asset?.length ? null : (
                  <div className="px-3 bg-white">
                    <p className="text-sm text-gray-500">
                      Search Results will be showing here{" "}
                    </p>
                  </div>
                )}
                ...
                {!!searchData?.data?.subCategory?.length ? (
                  <div className="mt-4 px-5">
                    ...
                    {searchData?.data?.subCategory?.map((subCategory) => (
                      <div
                        onClick={() => setCategoryId(subCategory?.category_id)}
                        className="mt-3"
                        key={subCategory?.id}
                      >
                        <div className="mb-3">
                        ...
                        <div className="mb-3">
                          <div className="flex gap-2 items-center ">
                            ...
                            <div>
                              <Link
                                onClick={handleClose}
                                href={`/${slugify(asset?.sub_category?.category?.name || asset?.subCategory?.category?.name)}/${slugify(asset?.sub_category?.name || asset?.subCategory?.name)}/${slugify(asset?.name)}-${asset?.id}`}
                              >
                                <p
                                  className="text-xs text-gray-800 cursor-pointer  border-b hover:border-primary"
                                  onClick={() => setIsSearchOpen(false)}
