    ...
    <div className="relative">
      <nav className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 text-slate-100 px-3 md:px-4 shadow-sm py-2 md:py-1 h-auto md:h-12">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center">
              <div className="hidden lg:flex items-center gap-3 xl:gap-5">
                ...
                <p className="font-semibold cursor-pointer text-slate-100 flex items-center text-xs xl:text-sm px-2 xl:px-3 py-2 bg-slate-800/20 hover:bg-slate-800 rounded-lg">
                ...
                <p className="font-semibold cursor-pointer text-slate-100 flex items-center text-xs xl:text-sm px-2 xl:px-3 py-2 bg-slate-800/20 hover:bg-slate-800 rounded-lg">
                  <Link href="https://blog.sketchshaper.com/">Blog</Link>
                </p>
              </div>

              <div
                className="hidden md:block relative ml-2 lg:ml-5 menu-dropdown"
                          ...
                          {expandedCategory === category.id && (
                            <div className="bg-slate-900 border border-slate-700">
                              {category.sub_categories?.map((subCategory) => (
                                ...
                                >
                                  <Link
                                    href={`/${slugify(category?.name)}-${category?.id}/${slugify(subCategory?.name)}-${subCategory?.id}`}
                                  >
                                    <div className="flex items-center justify-between px-8 py-1 text-sm text-slate-300 hover:bg-slate-700/50 hover:text-slate-100 cursor-pointer">
                                      ...
                                      <IoIosArrowDown
                                        className={`w-3 h-3 transition-transform ${expandedSubcategory === subCategory.id ? "rotate-180" : ""}`}
                                      />
                                    </div>
                                  </Link>
        ...
        <div className={`md:hidden ${isOpen ? "block" : "hidden"}`}>
          <div
            className={`fixed inset-y-0 left-0 z-50 w-4/5 sm:w-3/4 md:w-1/2 lg:w-1/3 bg-slate-900 text-slate-100 transform transition-transform ease-in-out duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
          >
