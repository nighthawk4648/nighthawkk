...
export const AllProFiles = () => {
    ...
    const fetchAllFiles = async (page, limit) => {
        try {
            const response = await fetch(
                `${API_BASE_URL}/sketchshaper-pro-files/pages?page=${page}&limit=${limit}&order=desc`
            );
    ...
    const { data: files, isLoading: filesLoading, hasMore, error: filesError, observerTarget } = useInfiniteScroll(fetchAllFiles, 12);
            ...
            <div className='mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4'>
                {filteredFiles.map((file) => {
                    ...
                    return (
                        ...
                        >
                            <div
                                className='relative w-[400px] h-[400px] rounded-lg overflow-hidden cursor-zoom-in'
                                onMouseMove={(e) => {
                            ...
                            <button
                                ...
                                className='mt-4 bg-blue-600 hover:bg-blue-700 hover:scale-105 hover:shadow-xl text-white text-sm font-semibold py-2 px-5 rounded-lg shadow-md transition-all duration-300 cursor-pointer disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center w-full'
                            >
